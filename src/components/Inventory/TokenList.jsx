...
export default function TokenList() {
  ...
  const selectDesign = async (id) => {
    setLoading(id);
    try {
      const { data } = await api.patch("/player/token/design", {
        designId: id,
      });
  ...
  const doPurchase = async (id, price) => {
    setLoading(id);
    try {
      const { data } = await api.post("/player/token/purchase", {
        designId: id,
        cost: price,
      });
      setPlayer(data.player);
  ...
  const owned = (id) =>
    id === "default" || player?.ownedTokenDesigns?.includes(id);

  const selected = (id) =>
    id === "default" ? !player?.tokenDesign : player?.tokenDesign === id;

  const imgSrc = (id) =>
    id === "default" ? `/tokens/red-64.png` : `/tokens/${id}/red-64.png`;
  const previewSrc = (id) =>
